...
run     bash -c 'echo "exec openbox-session &" >> ~/.xinitrc' && \
        ...
        bash -c 'echo "novnc --listen 6080 --vnc localhost:5900" >> ~/.xinitrc' && \
        bash -c 'chmod 755 ~/.xinitrc' 

# not sure why, but fix the resolv.conf with our DNS server
#run	bash -c 'echo "# Generated by NetworkManager" > /etc/resolv.conf && \
#        bash -c 'echo "search revoweb.home home" >> /etc/resolv.conf && \
#        bash -c 'echo  "" >> /etc/resolv.conf && \
#        bash -c 'echo "nameserver 172.16.16.1" >> /etc/resolv.conf 
...
RUN chmod +x /root/.vnc/xstartup /root/scripts/*.sh 
